...
KiB を超えた場合は、その frame の publish を捨てて送信キューの回復を待ちます。
...
- Kaguya: WebXR session の開始、`WebXRManager` binding、controller/hand event、pinch gesture、XR interaction
...
通常はVRモードで起動します。パススルーを試す場合は、URLに `xr=ar` を付けて開きます。

```text
https://<client-host>:5173/?xr=ar
```

パススルーはWebXRの `immersive-ar` セッションを使うため、対応するXRブラウザ・端末が必要です。
アプリがカメラ映像を直接取得するのではなく、端末が提供する現実世界の背景にThree.jsのシーンを重ねます。
