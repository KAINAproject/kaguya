...
`pnpm dev` の前に MoonBit の Wasm を自動でビルドし、`public/kaguya.wasm` に配置します。MoonBit が PATH にない場合は、リポジトリルートで `nix develop` に入ってから実行してください。
...
`tsc --noEmit` による型チェック、MoonBit の Wasm build、Vite の production build を実行します。
