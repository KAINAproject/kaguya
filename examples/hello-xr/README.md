...
WebXR と WebXR/WebGPU Binding に対応したブラウザでは、「VR を開始」ボタンから immersive VR session を開始できます。XR 中は各 eye の projection layer に pose の行列を使って、回転する立方体をステレオ描画します。WebXR/WebGPU Binding がないブラウザでは、標準の `XRWebGLLayer` と WebGL にフォールバックします。その場合、ボタンには「VR を開始（WebGL）」と表示されます。
