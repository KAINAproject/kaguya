...
WebXR と WebXR/WebGPU Binding に対応したブラウザでは、「VR を開始」ボタンから immersive VR session を開始できます。XR 中は各 eye の projection layer に pose の行列を使ってステレオ描画します。コントローラーが接続されている場合は、`targetRaySpace` に沿ったレイで奥の Cube を狙い、select で選択している間はコントローラーの 6 軸移動（位置と回転）に追従させられます。回転は Cube 自身を中心に行い、select を離すとその場所と向きに Cube が残ります。WebXR/WebGPU Binding がないブラウザでは、標準の `XRWebGLLayer` と WebGL にフォールバックします。その場合、ボタンには「VR を開始（WebGL）」と表示されます。
